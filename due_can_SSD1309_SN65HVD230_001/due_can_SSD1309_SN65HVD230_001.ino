...
const byte limiteBajoSOC = 20;     // 20 Límite para el SOC más bajo
const byte histeresisAlto = 55;    // 25 Histéresis SOC más bajo
...
const byte marcaBaja = 70;         // 70 Este valor también condiciona a partir de carga voltajeMaximo
...
// IDs CAN a observar (ATENCIÓN SOLO LEE LOS PRIMEROS 7)
const uint16_t canIDs[] = { 0x3C8, 0x3CB, 0x03B, 0x529, 0x3CA, 0x244, 0x039 };
...
void setup() {
  ...
  // Configura los filtros para los IDs de interés
  for (uint16_t id : canIDs) {
    Can0.watchFor(id);
  }
...
void error() {  // Borra los códigos de error del vehículo (Toyota Prius)
...
void activaEv(CAN_FRAME& canMsg) {
  ...
  if (EV == "NO") {
    // Verificar la velocidad del vehículo y la temperatura
    if (canMsg.id == 0x3CA && activacionPendiente == false && canMsg.data.byte[2] <= 0x2D && hvSocValue > 50 && temperatura >= 50) {
      // Verificar si la velocidad es igual o inferior a 45 km/h en hexadecimal y la temperatura es mayor o igual a 50
