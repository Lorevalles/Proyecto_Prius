...
const byte repMensaje = 1;       // Número repeticiones enviado el mensaje.
...
void setup() {
  ...
  // Configura los filtros para los ID de interés (son los que se van a leer.)
...
void mensajeCarga() {
  ...
  uint8_t data4[] = { 0x0F, 0x0F, 0x00, 0xE2, 0x40 };
...
void mensajeDescarga() {
  ...
  uint8_t data4[] = { 0x00, 0xC9, 0x00, 0xE2, 0xEB };
...
void mensajeNeutro() {
  ...
  uint8_t data4[] = { 0x00, 0x05, 0x00, 0xE2, 0x27 };
...
void voltajePaquete() {
  ...
  if (ultimo3CD == 0x3CD) {
    uint8_t data4[] = { 0x00, 0x00, 0x00, 0xE1, 0xB6 };

    for (int i = 0; i < repMensaje; i++) {  // Número repeticiones enviando el mensaje.
      // Envía el mensaje
      sendCANMessage(0x3CD, 5, data4);
      sentMessageCount++;
      ultimo3CD = 0xFFFF;
    }
  }
...
/*
// Función para manejar la recepción de mensajes CAN
void recibirMensajeCAN(uint32_t id, uint8_t length, uint8_t* data) {
  if (id == 0x3C9) { // Verifica si el mensaje recibido tiene el ID 0x3C9
    // Enviar el mensaje de respuesta inmediata
    uint8_t dataRespuesta[] = { 0x03, 0xFF, 0x25, 0x02, 0x9A, 0x03, 0x22, 0xBC };
    sendCANMessage(0x3C9, 8, dataRespuesta);
    sentMessageCount++;
  }
}
*/
...
void cargaPotencia() {
  ...
  if (ultimo3C9 == 0x3C9) {  // Verifica si el mensaje recibido tiene el ID 0x3C9
                             // Enviar el mensaje de respuesta inmediata
                             // Mensaje CAN ID 3C9 (11 bits), 8 03 FF 25 02 9A 03 22 BC
    uint8_t data7[] = { 0x03, 0xFF, 0x25, 0x02, 0x9A, 0x03, 0x22, 0xBC };
    for (int i = 0; i < repMensaje; i++) {  // Número repeticiones enviando el mensaje.
      // Envía el mensaje
      sendCANMessage(0x3C9, 8, data7);
      sentMessageCount++;
      ultimo3C9 = 0xFFFF;
    }
  }
...
// Mensajes que da el BMS (El BMS no recibe nada)
...
void leerCan() {
  ...
  if (Can0.available()) {
    ...
    if ((incoming.id == 0x3CB && incoming.length == 7) || (incoming.id == 0x03B && incoming.length == 5) || (incoming.id == 0x529 && incoming.length == 7) || (incoming.id == 0x3CA && incoming.length == 5) || (incoming.id == 0x244 && incoming.length == 8) || (incoming.id == 0x3C9 && incoming.length == 8) || (incoming.id == 0x3CD && incoming.length == 5) || (incoming.id == 0x4D1 && incoming.length == 8) || (incoming.id == 0x3C8 && incoming.length == 5)) {
      ...
      // Verifica la integridad de los datos utilizando un método de checksum
      if (verificarChecksum(incoming)) {
        // Llama a tu función de procesamiento para procesar el mensaje CAN
        processCanMessage(incoming);
      } else {
        // Datos corruptos, puedes tomar alguna acción aquí
        Serial.println("Error de checksum. Datos corruptos.");
      }
    }
  }
}
bool verificarChecksum(const CAN_FRAME& frame) {
  // Verifica si la longitud del mensaje es al menos 1 byte (el byte del checksum)
  if (frame.length < 1) {
    return false;
  }

  // Calcula el checksum sumando todos los bytes anteriores al byte del checksum
  ...
  for (int i = 0; i < frame.length - 1; i++) {
    ...
    calculatedChecksum += frame.data[i];
  }

  // Toma el complemento a dos del resultado
  calculatedChecksum = 0xFF - calculatedChecksum + 1;

  // Imprime el valor calculado del checksum
  Serial.print("calculatedChecksum: ");
  Serial.println(calculatedChecksum);

  // Compara el checksum calculado con el valor en el mensaje CAN
  return calculatedChecksum == frame.data[frame.length - 1];
}
...
void activaEv(CAN_FRAME& canMsg) {
  ...
  if (EV == "NO") {
    ...
    if (canMsg.id == 0x3CA && canMsg.data.byte[2] <= 0x31 && hvSocValue > 50) {
...
void controlSoc() {
  ...
  if (lastHvSocValue <= marcaBaja) {
    ...
    // Comprueba si hay más datos CAN recibidos
    if (Can0.available()) {
      Can0.read(incoming);
