...
const byte voltajeMinimo = 224;  // 230
...
const byte repMensaje = 2;       // Número repeticiones enviado el mensaje.

// Variables para el control de la salida A0 (Rele Voltaje)
unsigned long lastOutputA0ChangeMillis = 0;  // Último cambio de estado
const unsigned int tiempoActivoA0 = 15000;   // 0.5 segundos en milisegundos
const unsigned int tiempoInactivoA0 = 5000;  // 30 segundos en milisegundos
...
void processCanMessage(CAN_FRAME& incoming) {
  ...
  if (incoming.id == 0x3CB && incoming.length == 7) {
    ...
    if (incoming.id == 0x3CB && digitalRead(interruptorPin) == HIGH && (hvSocValue < marcaBaja || hvSocValue > marcaAlta)) {
...
void controlSoc() {
  ...
  if (lastHvSocValue <= marcaBaja) {
    ...
    while (Can0.available()) {
      ...
      // Procesar mensajes con ID 0x03B
      if (incoming.id == 0x03B && incoming.length == 5) {
        ultimoMensajeRecibidoID = 0x03B;
        mensajeCarga();
      }
      // Procesar mensajes con ID 0x3CD
      if (incoming.id == 0x3CD && incoming.length == 5) {
        ultimo3CD = 0x3CD;
        voltajePaquete();
      }
      // Procesar mensajes con ID 0x4D1
      if (incoming.id == 0x4D1 && incoming.length == 8) {
