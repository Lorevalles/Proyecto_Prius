...
void setup() {
  ...
  delay(tiempoEspera);
  //digitalWrite(outputPinA1, LOW);  // Desactiva el relé
  u8g2.clearBuffer();
  u8g2.firstPage();
  u8g2.clearDisplay();
  u8g2.setFont(u8g2_font_6x10_tf);
...
void processCanMessage(CAN_FRAME& incoming) {
  ...
  if (incoming.id == 0x3CB && incoming.length == 7) {
    ...
    // Ejecutar mensajeSOC bajo las condiciones específicas si permitirMensajeSOC es true
    if (permitirMensajeSOC && hvSocValue >= limiteBajoSOC) {
      // Primera condición específica
      if (digitalRead(interruptorPin) == HIGH) {
        mensajeSOC();
      }
      // Segunda condición específica
      else if ((hvSocValue < marcaBaja || hvSocValue > marcaAlta) || hvSocValue > socAlto) {
        mensajeSOC();
      }
    }
...
void controlSoc() {
  ...
  if (lastHvSocValue <= marcaBaja) {
    d3Activated = true;
    digitalWrite(outputPinD3, HIGH);  // Relé control voltaje batería
                                      /*
