...
(no he anulado su control, sigue siendo compatible con el)
  ...
  utilizando una pantalla SSD1322 y un módulo CAN
...
U8G2_SSD1322_240X128_F_4W_SW_SPI u8g2(U8G2_R0, /* clock=*/76, /* data=*/75, /* cs=*/10, /* dc=*/9, /* reset=*/8);

// Método para enviar comandos específicos al controlador SSD1322
void sendCommand(uint8_t command) {
  u8g2.sendF("c", command);
}

void sendCommandWithArgs(uint8_t command, uint8_t arg1, uint8_t arg2) {
  u8g2.sendF("c", command);
  u8g2.sendF("c", arg1);
  u8g2.sendF("c", arg2);
}
...
const float factor = 1.089;        // Factor de multiplicación por resistencia del simulador de batería
const byte socBajo = 70;           // 70 Valor a mantener mínimo de SOC. A partir de este valor Relé activado
const byte limiteBajoSOC = 25;     // 20 Límite para el SOC más bajo
...
const byte marcaBaja = 70;         // 70 Este valor también condiciona a partir de carga voltajeMáximo
const byte marcaAlta = 80;         // 80 Marca en ícono SOC
const byte repMensaje = 2;         // 2 Número de repeticiones enviando el mensaje.
const byte volLimBajoBat = 192;    // Límite de voltaje mínimo de la batería para dejar de enviar mensaje SOC
const byte volLimHiteresis = 200;  // Histéresis Límite de voltaje mínimo
...
const unsigned int tiempoActivoA0 = 1000;    // 1 segundo en milisegundos
...
const uint8_t batteryIconWidth = 240;
...
int decimalValue = 0;  // Acelerador posición
...
void setup() {
  ...
  // Ajustar la frecuencia del display
  sendCommand(0xB3);  // Establecer divisor de reloj
  sendCommand(0x91);  // Valor ajustado para reducir la frecuencia
  
  // Ajustar la configuración de pre-carga y corriente de conducción
  sendCommand(0xB1);  // Establecer el período de pre-carga
  sendCommand(0x32);  // Valor ajustado para el período de pre-carga
  
  // Ajustar la segunda configuración de pre-carga y mejorar la configuración de la pantalla
  sendCommand(0xB4);  // Ajustar la segunda pre-carga
  sendCommand(0xA0);  // Valor ajustado para la segunda pre-carga
  sendCommand(0xFD);  // Valor ajustado para mejorar la configuración de la pantalla
  
  // Ajustar la configuración de voltaje de pre-carga
  sendCommand(0xBB);  // Establecer el voltaje de pre-carga
  sendCommand(0x3A);  // Valor ajustado para el voltaje de pre-carga
  
  // Ajustar la configuración de corriente de conducción
  sendCommand(0xC1);  // Establecer la corriente de conducción de contraste
  sendCommand(0xFF);  // Valor ajustado para la corriente de conducción máxima
  
  // Ajustar la configuración de la fase de desbloqueo
  sendCommand(0xD9);  // Establecer la fase de desbloqueo
  sendCommand(0xF1);  // Valor ajustado para la fase de desbloqueo
  ...
  Can0.watchFor(0x244);  // ID 0x244, DLC 8, Acelerador
...
void drawCurrentIcon(uint8_t y, int value) {
  ...
  int barWidth = map(abs(value), 0, maxCurrent, 0, 240);  // Ancho adaptado a la nueva resolución

  // Dibujar la barra
  u8g2.drawFrame(0, y, 240, barHeight);
  if (value >= 0) {
    u8g2.drawBox(0, y, barWidth, barHeight - 1);
  } else {
    u8g2.drawBox(240 - barWidth, y, barWidth, barHeight - 1);
  }
}
...
void drawThrottleBar(uint8_t decimalValue) {
  ...
  const int baraWidth = 240 - baraX * 2;
...
void dibujarNivelCarga(int cargaModuloBat) {
  ...
  const int anchoBarra = 240 - posXBarra * 2;  // Ancho de la barra de carga
...
void updateu8g2() {
  ...
  if (readingsPerSecond > 0) {
    ...
    u8g2.print(" W");
    u8g2.setCursor(120, 54);  // Ajustar la posición X si es necesario
    u8g2.print("R:");
    ...
    u8g2.setCursor(120, 64);  // Ajustar la posición X si es necesario
    u8g2.print("RM:");
    u8g2.print(rpmValor);
    u8g2.setCursor(0, 18);
    u8g2.print("SOC:");
    ...
    u8g2.setCursor(120, 33);  // Ajustar la posición X si es necesario
    u8g2.print("V:");
    u8g2.print(voltajeTotal);
    u8g2.setCursor(80, 33);  // Ajustar la posición X si es necesario
    u8g2.print("EV:");
    u8g2.print(EV);
    u8g2.setCursor(200, 33);  // Ajustar la posición X si es necesario
    u8g2.print(outputA0Activated ? "Baj" : "Sub");
    u8g2.setCursor(190, 18);  // Ajustar la posición X si es necesario
    ...
    u8g2.setCursor(120, 18);  // Ajustar la posición X si es necesario
    ...
    u8g2.setCursor(220, 18);  // Ajustar la posición X si es necesario
...
void processCanMessage(CAN_FRAME& incoming) {
  ...
  if (incoming.id == 0x244 && incoming.length == 8) {
    ...
    decimalValue = byte6;  // Convertir valor hexadecimal a decimal
  }
}
...
void activaEv(CAN_FRAME& canMsg) {
  ...
  tiempoSinDatos = currentMillisEv - tiempoUltimaAplicacion;  // Calcular el tiempo desde la última recepción de datos

  if (canMsg.id == 0x529) {
    if (canMsg.data.byte[4] == 0x40) {
      EV = "SI";  // Si can_id es 0x529 y data[4] es 0x40, establece el mensaje en "SI"
    } else {
      EV = "NO";  // Si can_id es 0x529 pero data[4] no es 0x40, establece el mensaje en "NO"
    }
  }
  ...
  if (EV == "NO") {
    if (canMsg.id == 0x3CA && activacionPendiente == false && canMsg.data.byte[2] <= 0x2D && hvSocValue > 50) {
      if (activacionPendiente || tiempoSinDatos >= tiempoEsperar) {
        ultimaActivacion = currentMillisEv;  // Inicia la activación y guarda el tiempo
  ...
  if (activacionPendiente && currentMillisEv - ultimaActivacion >= evTiactivado) {
    digitalWrite(outputPinA1, LOW);  // Si ha pasado el tiempo de activación, establece en LOW
...
void conCarga() {
  ...
  static bool lastOutputA0State = false;  // Variable para realizar un seguimiento del último estado de salida A0
...
void lecSegundo() {  // Actualizar el recuento de mensajes enviados por esperaRef
  ...
  if (currentMillislec - previousSecondMillis >= esperaRef) {
    ...
    sentMessagesPerSecond = sentMessageCount;  // Actualizar valores de lecturas y mensajes enviados en el LCD
...
void checkConnectionStatus() {
  ...
  if (readingsPerSecond > 0) {
    ...
    u8g2.clearBuffer();                        // Muestra el mensaje de error
    u8g2.setFont(u8g2_font_ncenB10_tr);
...
void loop() {
  int buttonState = digitalRead(botonPin);  // Lee el estado actual del botón.

  if (buttonState == LOW && lastButtonState == HIGH) {  // Comprueba si el botón se ha pulsado
    error();  // Borra el error
  }

  lastButtonState = buttonState;  // Actualiza el estado anterior del botón y el interruptor
  ...
  if (tiempoActual - tiempoAnterior >= intervaloActualizacion) {  // Verifica si ha pasado 1 segundo desde la última actualización
    float potenciaInstantanea = voltajeTotal * PAValor;           // Calcula la potencia instantánea en vatios

    if (PAValor > 0) {
      energiaAcumulada += (potenciaInstantanea / 3600.0);  // Energía consumida: convertir vatios a vatios-hora
    } else if (PAValor < 0) {
      energiaRegenerada += (-potenciaInstantanea / 3600.0);  // Energía regenerada: convertir vatios a vatios-hora, PAValor es negativo
    }

    tiempoAnterior = tiempoActual;  // Actualiza el tiempo anterior para la próxima iteración
  }
}
