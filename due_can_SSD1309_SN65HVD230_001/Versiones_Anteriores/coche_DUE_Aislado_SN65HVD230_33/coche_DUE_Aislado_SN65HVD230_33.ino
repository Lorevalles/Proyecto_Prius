...
const byte voltajeMinimo = 245;    // 224
const byte voltajeMaximo = 246;    // 240
...
void updateu8g2() {
  ...
  if (voltajeV < volLimHiteresis && displayOn) {
    // Imprime el valor de 'voltajeModulo' con 2 decimales
    u8g2.print(voltajeModulo, 2);
...
void controlSoc() {
  ...
  else if (voltajeV >= voltajeMaximo && lastHvSocValue >= marcaBaja) {
    d3Activated = false;
    digitalWrite(outputPinD3, LOW);
  }
